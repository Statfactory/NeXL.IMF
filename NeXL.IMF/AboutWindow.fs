...
module AboutWindow =
    ...
    let create() =
        ...
        label1.Text <- "Excel Connector for IMF API. Powered by NeXL Technology"
        ...
        link.RequestNavigate.Add(fun args ->         
                                    try
                                        System.Diagnostics.Process.Start("https://github.com/Statfactory/NeXL.IMF") |> ignore
                                    with
                                        | _ -> ())
        link.NavigateUri <- new Uri("https://github.com/Statfactory/NeXL.IMF")
